...
export function normalizeUsState(token: string | null | undefined) {
  if (!token) {
    return null;
  }
  ...
  const exactCode = byCode.get(upper);
  if (exactCode) {
    return exactCode;
  }

  return byName.get(cleaned.toLowerCase()) ?? null;
}

export function getUsStateByCode(code: string) {
